...
### Changed — `supports_grammar` 미기록의 뜻 (agent-cli 9.24.0 짝)

agent-cli 가 부트 프로브를 없앴습니다(모델 서버가 바쁠 때 그 요청 하나에
인스턴스 열기가 수십 초 밀리는 것을 실측). 이제 키가 없으면 인스턴스의 📐 는
**미확인으로 잠기고**, admin 의 감지(프로브)를 돌리면 판정이 채워집니다. 편집
폼의 첫 항목 라벨·툴팁과 README 를 그 뜻으로 고쳤습니다. 동작 변경 없음.
...
사용자 제보: "게시글 제목을 쓰고 엔터를 치면 같은 이름으로 2개가 만들어질 때가
있네." **한글로 칠 때만** 재현되는 것이 단서였습니다 — 이중 발화 경로가 둘입니다.

- **IME 조합 확정.** 한글 조합을 Enter 로 확정하면 `keydown` 이 한 번
  (`isComposing=true`), 실제 제출로 또 한 번 — 둘 다 같은 제목을 읽습니다. 보드
  전체에 `isComposing` 검사가 하나도 없었습니다(agent-cli 쪽은 이미 쓰고 있습니다).
- **진행 중 가드 부재.** `create()` 가 입력을 `await` **뒤에** 비워서, 요청이 떠
  있는 동안 들어온 두 번째 호출이 같은 값을 그대로 통과시켰습니다. `submitClone()`
  도 같습니다 — `.disabled` 는 **버튼 클릭만** 막고 Enter 경로는 그대로 들어옵니다.

`tests/browser/test_create_post.py` 가 실브라우저로 두 경로를 다 재현합니다
(`AGENT_BOARD_BROWSER_TESTS=1`).

### Added — 릴리스 자산 자동 첨부

`.github/workflows/release.yml`: 릴리스를 발행하면 wheel + sdist 를 빌드해
붙입니다. 손으로 올리던 절차라 v1.29.0 은 sdist 가 빠져 있었습니다. 태그와
`agent_board.__version__` 이 어긋나면 올리기 전에 멈춥니다.

## [1.30.0] - 2026-08-30

> ⚠ **기존 설치는 마이그레이션이 필요합니다.** 워크스페이스 루트 이름과 post_id
> 형식이 둘 다 바뀝니다. 보드와 인스턴스를 **모두 정지한 뒤**
> `python scripts/migrate_short_post_ids.py` (드라이런) →  `--apply` 를 실행하세요.

### Changed — 워크스페이스 경로를 짧고 추적 가능하게

`post_id` 는 세 가지를 겸합니다: `posts` PK · 라우트 `/s/<post_id>` · **워크스페이스
디렉토리명**. `uuid4().hex`(32자)는 앞의 둘엔 문제없었지만 세 번째가 나빴습니다 —
디스크의 `cf2dad8ec6894e6583a72b0ba02fe98e` 가 어느 글인지 알 수 없었고, 절대경로가
파일 조작마다 에이전트 컨텍스트에 실려 들어가는데(경로는 `action_input` 으로 매 턴
재공급되고 compaction 파일 목록에도 누적) hex 는 토큰 효율이 나쁩니다.

- **post_id: 6자 소문자 base32** (`agent_board.ids`). Crockford 에서 혼동 글자
  `i`/`l`/`o`/`u` 를 뺀 32자 알파벳, `secrets` 추첨.
  - **소문자 전용** — 대소문자 무시 파일시스템(macOS/Windows)에서 서로 다른 PK 가
    **같은 디렉토리**가 되는 것을 막습니다.
  - **`random` 이 아니라 `secrets`** — 기본 `board-proxy` 게이트웨이는 `/s/<id>` 에
    인증이 없어 id 가 인스턴스를 가리는 유일한 가드입니다.
  - **엔트로피는 줄었습니다**: 32^6 ≈ 1.07e9(약 30비트, uuid4 의 122비트에서 감소).
    보드가 localhost 이거나 `caddy_basic_auth` 뒤에 있다는 전제에서 충분합니다.
    직접 노출한다면 `ids.ID_LENGTH` 를 올리세요 — 모든 소비자가 id 를 불투명
    문자열로 취급하므로 그 상수 하나만 바꾸면 됩니다.
  - **할당**: `store.create_post` 가 PK 충돌과 **디렉토리 선점**(삭제가 덜 끝나 남은
    고아 디렉토리) 양쪽을 확인하고 재추첨합니다. 고아 디렉토리를 새 글에 물려주면
    남의 파일을 상속하게 되므로.
- **기본 워크스페이스 루트: `<home>/workspaces` → `<home>/ws`.** 경로의 나머지 절반도
  매번 컨텍스트에 실리므로. `AGENT_BOARD_WORKSPACES` 오버라이드는 그대로입니다.
- 합쳐서 경로가 약 33자 짧아집니다:
  `…/data/workspaces/cf2dad8ec6894e6583a72b0ba02fe98e/` → `…/data/ws/k7m2xq/`.
- **`workspace_for()` 의 "파생, 미저장" 불변식은 유지**됩니다 — 매핑 컬럼을 새로
  저장하지 않으므로 사용자 입력 경로가 없고 traversal·공유 충돌 위험도 그대로 0.

### Added

- **대시보드 카드에 워크스페이스 id 칩** (`.ws-id`, 클릭 = 복사). 짧은 랜덤 id 는
  그 자체로 어느 글인지 말해주지 않으므로, 이게 없으면 이번 변경은 토큰만 줄이고
  **매핑 문제를 그대로 남깁니다**. 디스크의 디렉토리명을 카드에서 되짚을 수 있습니다.
- **`scripts/migrate_short_post_ids.py`** — 기존 보드를 새 형식으로 이전. 워크스페이스
  루트를 먼저 옮기고(옛 기본 이름일 때만; `AGENT_BOARD_WORKSPACES` 를 지정한 운영자의
  선택은 건드리지 않음), 그다음 글마다 디렉토리 rename → 행/스케줄 FK 갱신을 글 단위
  트랜잭션으로 처리합니다. 디렉토리를 **먼저** 옮겨서, 중간에 죽어도 DB 가 존재하는
  디렉토리를 가리키게 둡니다(그 반대보다 복구가 쉬움). 기본은 드라이런.
  세션 히스토리 안에 이미 기록된 절대경로는 고치지 않습니다(고아 참조로 남음).

### Fixed

- id 칩의 복사가 `navigator.clipboard` **거부** 시에도 `execCommand` 로 폴백합니다.
  종전 패턴은 API 부재만 폴백하고 거부는 무시해, 권한 없는 컨텍스트(LAN http,
  헤드리스)에서 클릭이 조용히 아무 일도 안 한 것처럼 보였습니다.
- (테스트) `tests/browser/conftest.int_from` 이 post_id 를 hex 로 가정하던 것 수정 —
  새 알파벳에서 `ValueError` 였습니다.
...
- **1.28.0 (Hypercorn TLS+HTTP/2+인증) 되돌림** — ASGI 서버를 다시 uvicorn 으로
  복귀. 1.28.0 에서 추가했던 Hypercorn 전환·`AGENT_BOARD_TLS_CERT/KEY`·컨트롤플레인
  인증(`auth.py`)·`/api/gateway` h2 필드를 모두 원복한다(`git revert`). 동작·의존성은
  1.27.0 과 동일. TLS/h2/인증 임베딩은 추후 재검토.

...
- **탭 가드 (v1.14.0, v1.15.0 에서 Web Locks)** — board-proxy 게이트웨이에서는 모든
  방이 한 origin 으로 프록시되고 방/대시보드 탭이 각각 SSE 연결 1개를 점유하는데,
  브라우저는 HTTP/1.1 origin 당 동시 연결이 **6개**(프로필 전체 합산)뿐이라 6개가 차면
  승인 클릭까지 모든 요청이 조용히 멈춘다. 연결을 잡는 탭은 **연결 슬롯 락**
  (`agentcli-conn-slot-0..4`, Web Locks — agent-cli ≥ 7.6.0 프로토콜)을 원자적으로
  보유하고 탭이 닫히면 브라우저가 즉시 해제 — 열기 게이트는 `locks.query()` 정확값으로
  **4개째 경고, 5개째 차단**(6이 되는 순간 포화라 1슬롯 예약). 대시보드 자신도 슬롯을
  잡아야 실시간 갱신 SSE 를 열고, 슬롯이 없으면 목록만 정적 표시 + 슬롯이 나는 순간
  자동 연결. 같은 글을 다시 열면 새 탭 대신 **기존 창을 재사용**(named window — 연결이
  안 늘어 가드 면제). gateway=caddy(h2) 모드에선 가드가 자동 해제된다(`/api/gateway`).
  board 밖 진입(URL 직접·세션 복원·탭 복제)은 agent-cli(≥7.5.0) 쪽 입장 게이트가 같은
  슬롯으로 파킹·자동 입장 처리.

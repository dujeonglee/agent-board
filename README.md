...
- **탭 가드 (v1.16.0)** — board-proxy 게이트웨이에서는 모든 방이 한 origin 으로
  ...
  **기존 창을 재사용**(named window — 연결이 안 늘어 가드 면제). gateway=caddy(h2)
  ...
  수 있고, 그때는 탭을 닫으면 된다).
- **대화방 clone (v1.20.0)**: 새 글 만들 때 `📋 복제` 로 다른 글을 원본 선택 → 워크스페이스 파일 트리에서 복사할 항목 체크. `.agent-cli/sessions/…` 를 포함하면 **대화까지 이어받고**(세션 remap: 새 sid + `_meta` 재작성, stale 사이드카 제외), 안 고르면 **파일만 가져온 새 대화**. 한 메커니즘이 "전체 복제 / 파일만 / 임의 선택"을 다 덮는다.
